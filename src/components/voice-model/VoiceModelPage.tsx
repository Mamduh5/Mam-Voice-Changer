...
export function VoiceModelPage({
  ...
  const profileId = profiles.selectedProfileId;
  const manifest =
    dataset.status.manifest?.profile.id === profileId ? dataset.status.manifest : profiles.manifest;
  ...
  const selectedArtifact =
    ...
    profileArtifacts[0] ??
    null;
  const consentActive = Boolean(manifest?.consent.consentConfirmed && !manifest?.consent.revokedAt);
  const busy = disabled || profiles.busy || dataset.busy || models.busy;
    ...
    <div className="page-stack voice-model-page">
      ...
      {!profileId && (
        <div className="dataset-empty">
          <h3>Select a consent-active voice profile before creating or using a model.</h3>
          <button type="button" onClick={onOpenProfiles}>
            Open Profiles
          </button>
        </div>
      )}
      <div className="model-master-detail">
        ...
        <div className="workspace-main">
          ...
          <ModelPortabilityPanel
            ...
            consentVersion={consentActive ? (manifest?.consent.consentVersion ?? null) : null}
            busy={busy}
