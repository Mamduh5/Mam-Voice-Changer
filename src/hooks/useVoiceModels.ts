...
export function useVoiceModels(enabled: boolean) {
  ...
  return {
    ...
    startTraining: (profileId: string, snapshotId: string, configuration: TrainingConfiguration) =>
      run(() => tauriAudioApi.startVoiceModelTraining(profileId, snapshotId, configuration)),
